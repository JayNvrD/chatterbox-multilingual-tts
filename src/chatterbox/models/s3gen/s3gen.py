...
class S3Token2Mel(torch.nn.Module):
    ...
    def forward(
        ...
        finalize: bool = False,
        n_cfm_timesteps: int = 4,  # HYBRID OPT: Reduced from 10 to 4
    ):
        """
        ...
        output_mels, _ = self.flow.inference(
            ...
            finalize=finalize,
            n_timesteps=n_cfm_timesteps,  # HYBRID OPT: Pass through CFM steps
            **ref_dict,
        )
        return output_mels
...
class S3Token2Wav(S3Token2Mel):
    ...
    def forward(
        ...
        ref_dict: Optional[dict] = None,
        finalize: bool = False,
        n_cfm_timesteps: int = 4,  # HYBRID OPT: Reduced from 10
    ):
        output_mels = super().forward(speech_tokens, ref_wav=ref_wav, ref_sr=ref_sr, ref_dict=ref_dict, finalize=finalize, n_cfm_timesteps=n_cfm_timesteps)
    ...
    def flow_inference(
        ...
        ref_dict: Optional[dict] = None,
        finalize: bool = False,
        n_cfm_timesteps: int = 4,  # HYBRID OPT
    ):
        return super().forward(speech_tokens, ref_wav=ref_wav, ref_sr=ref_sr, ref_dict=ref_dict, finalize=finalize, n_cfm_timesteps=n_cfm_timesteps)
    ...
    def inference(
        ...
        finalize: bool = True,
        n_cfm_timesteps: int = 4,  # HYBRID OPT: Reduced from 10 for lower latency
    ):
